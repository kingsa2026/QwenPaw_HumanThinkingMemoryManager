...
### v1.0.2-beta0.2：智能记忆管理版本（最新）
**核心亮点**：
- **智能记忆管理**：实现记忆分类、记忆关联、记忆摘要、记忆优先级调整
- **数据库结构扩展**：添加记忆关联表、记忆分类表和分类关联表
- **版本管理改进**：修复版本号格式和版本比较函数
- **一键升级脚本**：添加从旧版本升级到新版本的一键脚本
- **基于v1.0.2-beta0.1重构**：在v1.0.2-beta0.1的基础上添加智能记忆管理功能
...
- **会话过滤**：搜索时根据session_id过滤结果，确保只返回特定会话的记忆
- **缓存机制**：实现了基于大小、数量和时间的缓存管理，防止记忆丢失

### v1.0.4：缓存优化版本
**核心亮点**：
- **缓存限制**：添加了缓存大小限制（100000字符）和数量限制（10条）
- **超时自动刷新**：实现了无活动超时自动刷新缓存（300秒）
- **批量写入**：优化了批量写入机制，提高高频率会话场景下的性能
- **错误恢复**：在缓存写入失败时自动恢复缓存，防止记忆丢失

### v1.0.3：性能优化版本
**核心亮点**：
- **批量存储**：实现批量存储记忆功能，减少数据库操作次数
- **内存缓存**：添加内存缓存机制，提高记忆存取速度
- **实时查询**：支持实时查询内存缓存中的记忆
- **异步写入**：实现异步批量写入，提高高频率会话场景下的性能

### v1.0.2：集成版本
**核心亮点**：
- **UI集成**：支持通过QwenPaw UI控制台选择记忆管理后端
- **自动安装**：提供自动安装和卸载脚本，简化安装过程
- **版本兼容**：与QwenPaw最新版本完全兼容
- **配置集成**：改进配置文件集成，支持通过配置文件和环境变量进行配置

### v1.0.1：搜索优化版本
**核心亮点**：
- **类人搜索**：实现模拟人类思考方式的搜索机制
- **智能检索**：关键词加就近时间向量搜索，提高搜索准确性
- **权重调整**：搜索频率记录与自动权重调整，优先返回重要记忆
- **上下文优化**：优化上下文流畅性，提升搜索结果质量

### v1.0.0：初始版本
**核心亮点**：
- **SQLite存储**：使用SQLite本地存储，无需额外依赖
- **记忆管理**：实现记忆冷藏/解冻机制，优化存储空间
- **向量检索**：支持向量检索，提高搜索准确性
- **数据迁移**：支持从旧格式（MEMORY.md, memory.json）迁移到新的SQLite格式
- **版本管理**：内置版本管理机制，支持数据库结构升级
